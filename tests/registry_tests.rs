...
impl Command for AliasCmd {
    ...
    fn aliases(&self) -> &[&str] { &["alt", "p"] }

    fn help(&self) -> Option<&str> { Some("Alias test command") }

    fn validate(&self, _args: &[String]) -> Result<(), String> { Ok(()) }

    fn execute(&self, _args: &[String]) { log_exec("alias_executed") }
}
...
fn alias_resolution_executes_primary_command() {
    ...
    assert!(log.contains(&"alias_executed"), "alias did not route to primary command");
}
