...
impl Command for FrameworkCommand {
    ...
    fn execute(&self, _args: &[String]) {
        ...
        let framework = build()
            .part("Mod").color(LIGHT_BLUE).bold()
            .part("cli").color(BLUE)
            .part(":").color(GREY).space()
            .part("version:").space()
            .part(modcli_version()).bold()
            .get();

        // Construct framework description
        let description = build()
            .part("⬢").color(BLUE).space()
            .part("cli framework for").italic().space()
            .part("Rust").italic().bold().color(GREY).space()
            .get();
