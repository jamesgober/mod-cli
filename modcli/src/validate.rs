...
pub fn require(argsv: &[String], name: &str) -> Result<(), ModCliError> {
    if args::flag(argsv, name) || args::get_string(argsv, name).is_some() {
        ...
        Err(ModCliError::InvalidUsage(format!("missing required argument: {name}")))
    }
}

/// Parse a typed value from `--key`, ensuring it falls in an optional inclusive range.
pub fn parse_in_range<T>(argsv: &[String], name: &str, min: Option<T>, max: Option<T>) -> Result<T, ModCliError>
where
    T: FromStr + PartialOrd + Copy + std::fmt::Display,
{
    ...
    if let Some(lo) = min {
        if val < lo {
            return Err(ModCliError::InvalidUsage(format!("{name} below minimum ({val} < {lo})")));
        }
    }
    if let Some(hi) = max {
        if val > hi {
            return Err(ModCliError::InvalidUsage(format!("{name} above maximum ({val} > {hi})")));
        }
    }
    Ok(val)
}
...
pub fn path_exists(p: &str) -> Result<(), ModCliError> {
    if Path::new(p).exists() {
        ...
        Err(ModCliError::InvalidUsage(format!("path does not exist: {p}")))
    }
}
